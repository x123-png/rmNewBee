...
Task::Task(int id,std::string task_desc):m_id(id),m_task_desc(task_desc),m_task_status(" "){
      //外部启用需要使用作用域解析运算符，将任务id初始化为0
}

void Task::executeTask() {
    m_task_status="已执行";  //ִ执行任务函数用于更新任务状态
}
...
std::string Task::getDesc(std::string command){//添加任务指令
    m_task_desc=command;
    return m_task_desc;    
}

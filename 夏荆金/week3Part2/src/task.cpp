...
Task::Task(int id,std::string task_desc,int priority):m_id(id),m_task_desc(task_desc),m_task_status("unexecuted"),m_priority(priority){
...
void Task::addStatus(std::string statu){
 ...
 bool Task::cmp(const Task& t1,const Task& t2){
    return t1.m_priority>t2.m_priority;//降序排列
}